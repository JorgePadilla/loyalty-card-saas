module AdminHelper
  def admin_nav_link(label, path, icon: nil)
    ...
    classes = if active
      "flex items-center gap-3 px-3 py-2 rounded-lg bg-white/10 text-white text-sm font-medium"
    else
      "flex items-center gap-3 px-3 py-2 rounded-lg text-white/60 hover:text-white hover:bg-white/5 text-sm transition-colors"
    end

    link_to path, class: classes do
      content_tag(:span, admin_icon(icon), class: "w-5 h-5") +
  ...
  def admin_icon(name)
    icons = {
      ...
      "zap" => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75Z" /></svg>',
  ...
  def stat_change_badge(value)
    if value > 0
      content_tag(:span, "+#{value}%", class: "text-xs text-success font-medium")
    elsif value < 0
      content_tag(:span, "#{value}%", class: "text-xs text-danger font-medium")
    else
      content_tag(:span, "0%", class: "text-xs text-secondary font-medium")
    end
  end
end
